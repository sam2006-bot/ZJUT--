...
一个基于 OpenAI 兼容代理协议的学生代码自动评分 Web 应用。教师可以设定作业要求，上传单个学生代码文件，或者直接上传包含整班作业的 ZIP 压缩包，系统会先运行测试样例，再交给 Claude 模型进行结构化评审、汇总统计并导出结果。
...
- **批量 ZIP 批改** — 支持上传包含整班学生代码的 ZIP 压缩包，系统会自动提取并逐份批改
- **结果汇总导出** — 批量批改完成后可一键导出总表、统计信息、JSON 明细和每位学生的 Markdown 报告
- **自动测试优先** — 支持对单个提交或整批提交统一运行测试样例，再把测试结果交给 AI 进行分析
...
- 支持上传 `.zip`、`.py`、`.c`、`.cpp`、`.cxx`、`.cc`、`.java`
- 上传单个源码文件时，流程与原来一致
- 上传 ZIP 时，程序会自动提取其中的源代码文件，并按以下规则分组批改：
  - 如果 ZIP 中是 `student1.py`、`student2.py` 这类顶层文件，则每个文件视为一份提交
  - 如果 ZIP 中是 `student1/main.py`、`student2/Main.java` 这类目录结构，则每个顶层学生目录视为一份提交
- 批量批改完成后，页面会显示：
  - 提交总数、成功数、失败数、平均分、分数分布
  - 自动测试汇总（如果你配置了测试样例）
  - 每位学生的详细批改报告
- 点击“导出全部结果”后，会下载一个 ZIP，包含：
  - `grading_results.csv`
  - `statistics.json`
...
|------|------|------|
| GET | `/` | 评分主页面 |
| POST | `/grade` | 提交单个代码文件或 ZIP 压缩包进行评分（返回 JSON） |
| GET | `/exports/<token>` | 下载最近一次批量批改导出包 |
| POST | `/login` | 邀请码登录 |
| GET | `/logout` | 注销并清除会话 |
| GET | `/healthz` | 健康检查 |
